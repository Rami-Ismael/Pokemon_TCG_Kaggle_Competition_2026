# Prompt for Claude Code — Build the Pokemon TCG BC Pipeline

Use this file as the instruction brief when asking Claude Code to scaffold the
implementation pipeline. The `## Open Questions` section is the acceptance
checklist: every box should be resolved by the generated code (or explicitly
...
**Role.** You are a senior ML engineer and Kaggle competition specialist. Write
clean, runnable **PyTorch** code for a **Behavioral Cloning (BC)** imitation-learning
...
## 📝 Changelog
- **Added Role/persona**: senior ML + Kaggle specialist, so the model owns the architecture decision.
- **Clarified track**: distinguished the Simulation (agent) track from the Strategy writeup track; flagged the ambiguity for you to confirm (`[USER TO FILL]`).
- **Added data ground truth**: the real JSON schema (steps, variable-length `option`, null opponent hand, held-out-day split) instead of vague "daily episode datasets".
- **Added explicit IL method**: named it **Behavioral Cloning** and noted DAgger as the natural follow-up.
- **Sharpened the architecture tradeoff into an instruction**: Transformer-vs-MLP/LSTM comparison made concrete, with the **CPU-only / 600 s overage eval budget** as the decisive constraint.
- **Added output format**: file structure (`data/model/train/agent/README`), the `agent(obs_dict)->list[int]` competition interface, and the **masked per-option scorer head** (not a fixed `N_ACTIONS` layer).
- **Added success criteria + edge cases**: legal-option masking, null opponent hand, deck-selection step, never-crash fallback, self-play validation.
- **Silently corrected** "immitation"→imitation, "transformes"→transformers (intent unchanged).

---

## Open Questions (acceptance checklist — resolve each)
...
### C. Imitation-learning methodology
