...
def main() -> None:
    ...
    ap.add_argument("--agent", default="il_agent",
                    help="target agent (any AGENT_FILES key, e.g. s2v2_e0_s43)")
    ap.add_argument("--opponent", default="rule_baseline",
                    help="opponent agent (any AGENT_FILES key)")
    args = ap.parse_args()

    from kaggle_environments import make

    il_fn = load_agent(args.agent)
    opp_fn = load_agent(args.opponent)

    stats: Counter = Counter()
    outcomes = []
    for i in range(args.games):
        env = make("cabt")
        a = make_logging_agent(f"{args.agent}_p0", stats, il_fn)
        b = make_logging_agent(f"{args.opponent}_p1", stats, opp_fn)
    ...
    print(f"\n--- aggregate behavioral stats across all games ({args.agent} as p0) ---")
    for k in sorted(stats):
        print(f"  {k}: {stats[k]}")

    n_decisions = stats.get(f"{args.agent}_p0/decisions", 0)
    print("\n--- sanity checks ---")
    print(f"  ever declined an optional selection? {'YES' if stats.get(f'{args.agent}_p0/declined', 0) > 0 else 'NO'} "
          f"({stats.get(f'{args.agent}_p0/declined', 0)} times)")
    print(f"  ever retreated? {'YES' if stats.get(f'{args.agent}_p0/retreated', 0) > 0 else 'NO'} "
          f"({stats.get(f'{args.agent}_p0/retreated', 0)} times)")
    print(f"  ever attacked? {'YES' if stats.get(f'{args.agent}_p0/attacked', 0) > 0 else 'NO'} "
          f"({stats.get(f'{args.agent}_p0/attacked', 0)} times)")
    ...
    print(f"\n  record vs {args.opponent}: {wins}W-{losses}L / {len(outcomes)} games (not statistically meaningful at this N -- see Q24)")
