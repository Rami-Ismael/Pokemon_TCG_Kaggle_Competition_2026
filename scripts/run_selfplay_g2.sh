...
LOG=runs/selfplay_g2.log
t0=$(date +%s)
while :; do
  start=$(date +%s)
  nice -n 5 $MAIN/.venv-ppo/bin/python scripts/train_ppo_puffer.py \
    --init-from models/selfplay_g1/u963584_final \
    --init-policy-full models/selfplay_g1/policy_full.pt \
    ...
    --run-tag selfplay_g2 --out models/selfplay_g2 >>"$LOG" 2>&1
  rc=$?
  ran=$(( $(date +%s) - start ))
  [ $rc -eq 0 ] && { echo "SELFPLAY G2 DONE rc=0" >>"$LOG"; exit 0; }
  if [ $ran -lt 120 ] && [ $(( $(date +%s) - t0 )) -lt 10800 ]; then
    echo "preflight/launch failed fast (rc=$rc after ${ran}s) -- retrying in 120s" >>"$LOG"
    sleep 120
    continue
  fi
  echo "SELFPLAY G2 FAILED rc=$rc after ${ran}s -- not retrying" >>"$LOG"
  exit $rc
done
