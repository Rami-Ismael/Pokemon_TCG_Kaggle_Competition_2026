...
sys.path.insert(0, str(Path("data/external/cg-lib")))
from cg.api import (
...
# ---- RUN PROOF ----
raw = json.loads(Path("notebooks/sample_obs.json").read_text())
res = agent(raw["obs_dict"])
print("AGENT RESULT (first 8):", res[:8])
print("num options:", len(raw["obs_dict"].get("select", {}).get("option", [])))
print("context:", raw["obs_dict"]["select"]["context"] if raw["obs_dict"].get("select") else None)
