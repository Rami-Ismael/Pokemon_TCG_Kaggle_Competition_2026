...
def main() -> None:
    ...
    ap.add_argument("--mirror-share", type=float, default=0.625,
                    help="share of games vs the LIVE mirror; the rest go to "
                         "the frozen --league checkpoints. Opponents are "
                         "lineage-only -- the public-pool bucket was removed "
                         "2026-08-05, not zeroed")
    ...
    print(f"league: {league}  mirror_share: {args.mirror_share}")
    ...
    for update in range(1, args.updates + 1):
        ...
        actor.to(device)
        tasks = [(i, sample_lineage_opponent(rng, league, args.mirror_share), i % 2)
        ...
        rows, outcomes, wins = [], [], {"mirror": [0, 0], "league": [0, 0]}
        n_fallbacks = 0
        for r in results:
            rows.extend(r["rows"])
            outcomes.extend([r["outcome"]] * len(r["rows"]))
            n_fallbacks += r["fallbacks"]
            bucket = "mirror" if r["opponent"][1] == "__mirror__" else "league"
        ...
        print(f"update {update}/{args.updates}  rows={n}  "
              ...
              f"win mirror={wr['mirror']} league={wr['league']}  "
