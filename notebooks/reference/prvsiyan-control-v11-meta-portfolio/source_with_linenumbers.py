...
def facing_crustle_race(opponent) -> bool:
    """A visible Crustle is a prize race, not a mill target.

    The old control list has no ex Pokémon for Neutralization Zone to blank,
    and its straightforward 120-damage clock beats passive one-card milling.
    Once it has actually appeared (board or discard only), turn Great Tusk
    into the 160-damage attacker rather than waiting for a library-out.
    """
    return CRUSTLE in opponent_visible_ids(opponent)
...
def should_ko_mode(me, opponent, state) -> bool:
    ...
    if facing_crustle_race(opponent):
        return True
...
build = WORK / 'ptcg_control_submission'
if build.exists():
    shutil.rmtree(build)
...
spec = importlib.util.spec_from_file_location('control_agent', build / 'main.py')
control_agent = importlib.util.module_from_spec(spec)
spec.loader.exec_module(control_agent)
...
def smoke_game(max_steps=600):
    ...
    try:
        for step in range(max_steps):
            ...
            selected = control_agent.agent(observation)
...
REFRESH_UTC = "2026-07-30T02:09Z"
CONTROL_VIS_DIR = WORK / "ptcg_control_v11_visuals"
CONTROL_VIS_DIR.mkdir(exist_ok=True)
CURRENT_OFFICIAL_ROWS = json.loads(r'''[
  {
    "public_score": 844.4,
    "role": "owned high-water",
    "row_id": 55056992,
    "status": "COMPLETE",
    "surface": "Observable meta-router"
  },
  {
    "public_score": 813.2,
    "role": "latest observed drift branch",
    "row_id": 55094054,
    "status": "COMPLETE",
  ...
  },
  {
    ...
    "row_id": 55086222,
    "status": "COMPLETE",
    "surface": "Observable V15 Articuno tech"
  },
  {
    "public_score": 678.3,
    "role": "below portfolio bar",
    "row_id": 55058197,
    "status": "COMPLETE",
    "surface": "Observable V4 detour"
  },
  {
    "public_score": 555.3,
  ...
  {
    "public_score": 531.3,
    "role": "failed official transfer",
    "row_id": 55078864,
    "status": "COMPLETE",
    "surface": "RMY V4a Azumarill/Jellicent"
  }
]''')
AZUMARILL_LOCAL_PANEL = json.loads(r'''{
  "archive_sha256": "15b4a4f2c904bcab5dd5bb7660885b5baa5dcb03dfac7ae5a02cafc210d2de90",
  "deck_rows": 60,
  "deck_sha256": "972d74001fa9c944b81c1fb1ed330fa9b07d36171053a2c7944b16de20168f4c",
  "draws": 1,
  "errors": 0,
  "games": 576,
  "gate": "LOCAL_PROXY_PASS_NOT_KAGGLE_SCORE",
  "label": "Azumarill/Jellicent local broad-successor V4a",
  "losses": 137,
  "main_sha256": "21d13802d1396c9b97403550604dff375440bcffbb03c12df0043aa134f447eb",
  "minimum_opponent_win_rate": 0.5625,
  "per_opponent_win_rates": {
    "anti_control": 0.90625,
    "borealis_1050": 0.609375,
    "crustle_dash": 0.828125,
    "exact_current_public_a81e": 0.5625,
    "exact_rmy_55057790": 0.921875,
    "field_router": 0.921875,
    "grimmsnarl": 0.5625,
    "highwater_55056992": 0.9375,
    "makthanithin_1084_5": 0.59375
  },
  "raw_win_rate": 0.7604166666666666,
  "submission_hold_reason": "Local SDK evidence is not a Kaggle score; requires successful public notebook run, exact downloaded-output validation, and an explicit APPROVE/HOLD gate before any UI-only official submission.",
  "wins": 438
}''')
GARCHOMP_LOCAL_PANEL = json.loads(r'''{
  "base_license": "Apache-2.0",
  "base_source": "https://www.kaggle.com/code/masamikobayashi/a-sample-cynthia-garchomp-ex-deck",
  "base_visible_official_score": null,
  "candidate_archive_sha256": "2d354903412d1b5de2d027f337b1d150ca2186f96d538a6a790261d8b6fbb877",
  "candidate_deck_sha256": "f6fe420cb34f07dd9445a6b6eef6043e67487a2fc1869b923cf5617443a8ecdd",
  "candidate_main_sha256": "687cb0776383064f7fdff4ce943a39896e012d50b9f41bb95f3b41bfc15ede33",
  "four_roserade_weight_followup": {
    "decision": "HOLD",
    "interpretation": "A tempting deck-count tweak, but still only local SDK evidence and not a promotion gate.",
    "seed_202607300130": {
      "candidate": "279/384",
      "control": "255/384",
      "delta_pp": 6.25,
      "min_anchor_delta_pp": -1.5625
    },
    "seed_202607300131": {
      "candidate": "270/384",
      "control": "261/384",
      "delta_pp": 2.34375,
      "min_anchor_delta_pp": -6.25
    }
  },
  "hold_reason": "Local proxy gains are broad but unstable against the strongest owned contextual branch and show large Alakazam/Archaludon regressions.",
  "label": "Apache-2.0 Garchomp current-meta local lane",
  "loader_gate": "PASS: absolute-final licensed_garchomp_current_meta_entrypoint returned exact 60-card deck in 3/3 fresh namespaces",
  "official_submission": false,
  "promotion_decision": "HOLD",
  "public_notebook_update": false,
  "source_script_version_id": 331254883,
  "stamp_guard_followup": {
    "decision": "ABANDON",
    "failed_anchor_regressions": {
      "crustle_rule": -6.25,
      "festival_rules": -2.34375
    },
    "pooled_candidate_wins": 541,
    "pooled_control_wins": 532,
    "pooled_delta_pp": 1.171875,
    "pooled_games_per_surface": 768
  },
  "vs_exact_parent_844": {
    "pooled_candidate_wins": 545,
    "pooled_control_wins": 453,
    "pooled_delta_pp": 11.979166666666664,
    "pooled_games_per_surface": 768,
    "seed_202607300101": {
      "candidate": "265/384",
      "control": "237/384",
      "delta_pp": 7.2916666666666625,
      "min_anchor_delta_pp": -35.9375
    },
    "seed_202607300102": {
      "candidate": "280/384",
      "control": "216/384",
      "delta_pp": 16.666666666666664,
      "min_anchor_delta_pp": -20.3125
    }
  },
  "vs_exact_v22_contextual": {
    "per_anchor_delta_pp": {
      "alakazam_rule": -23.4375,
      "archaludon_starmie": -30.46875,
      "crustle_rule": 21.875,
      "dragapult_rule_proxy": 10.9375,
      "festival_rules": 9.375,
      "grimmsnarl_observable_proxy": 1.5625
    },
    "pooled_candidate_wins": 552,
    "pooled_control_wins": 565,
    "pooled_delta_pp": -1.692708333333337,
    "pooled_games_per_surface": 768
  }
}''')
rows_df = pd.DataFrame(CURRENT_OFFICIAL_ROWS)
rows_df["delta_vs_floor"] = rows_df["public_score"] - float(rows_df.loc[rows_df["row_id"].eq(55000147), "public_score"].iloc[0])
rows_df["delta_vs_highwater"] = rows_df["public_score"] - float(rows_df["public_score"].max())
rows_df.to_csv(WORK / "ptcg_control_v11_score_context_20260730.csv", index=False)
display(rows_df)

palette = {"ink": "#111827", "slate": "#64748b", "blue": "#2563eb", "green": "#16a34a", "orange": "#f97316", "red": "#dc2626", "teal": "#0f766e"}

fig, ax = plt.subplots(figsize=(13.2, 5.0))
bar_colors = [palette["green"] if role == "owned high-water" else palette["blue"] for role in rows_df["role"]]
bars = ax.bar(rows_df["surface"], rows_df["public_score"], color=bar_colors)
ax.set_title("Control v11 portfolio: exact official-row context, not local Elo theater", loc="left", fontsize=14.5, fontweight="bold")
ax.set_ylabel("publicScore")
ax.set_ylim(520, 880)
ax.tick_params(axis="x", rotation=24)
ax.grid(axis="y", alpha=0.22)
for bar, row in zip(bars, rows_df.to_dict("records")):
    ax.text(bar.get_x() + bar.get_width() / 2, row["public_score"] + 7, f"{row['public_score']:.1f}\n#{int(row['row_id'])}", ha="center", fontsize=8.4)
ax.spines[["top", "right"]].set_visible(False)
fig.tight_layout()
fig.savefig(CONTROL_VIS_DIR / "01_exact_official_score_context.png", dpi=180, bbox_inches="tight")
plt.show()

fig, ax = plt.subplots(figsize=(12.4, 4.7))
plot_df = rows_df.sort_values("delta_vs_floor")
ax.barh(plot_df["surface"], plot_df["delta_vs_floor"], color=[palette["green"] if v > 0 else palette["slate"] for v in plot_df["delta_vs_floor"]])
ax.set_title("Campaign learning curve: what a new control surface must beat", loc="left", fontsize=14.5, fontweight="bold")
ax.set_xlabel("publicScore gain vs row 55000147")
for y, row in enumerate(plot_df.to_dict("records")):
    ax.text(row["delta_vs_floor"] + 4, y, f"+{row['delta_vs_floor']:.1f}", va="center", fontsize=9)
ax.spines[["top", "right"]].set_visible(False)
ax.grid(axis="x", alpha=0.22)
fig.tight_layout()
fig.savefig(CONTROL_VIS_DIR / "02_gain_vs_legacy_floor.png", dpi=180, bbox_inches="tight")
plt.show()

fig, ax = plt.subplots(figsize=(13.5, 4.4))
ax.axis("off")
cards = [
    ("Deck", "60 rows\nexact callback", palette["teal"]),
    ("Policy", "visible-state\nlegal options", palette["blue"]),
    ("Archive", "tar hash\nsafe members", palette["slate"]),
    ("Smoke test", "engine sanity\nnot a score", palette["orange"]),
    ("Official row", "UI submission\nonly after gates", palette["green"]),
]
for i, (head, body, color) in enumerate(cards):
    x = 0.08 + i * 0.212
    ax.text(x, 0.68, head, ha="center", va="center", color="white", fontsize=10, fontweight="bold", transform=ax.transAxes, bbox=dict(boxstyle="round,pad=.55", fc=color, ec="none"))
    ax.text(x, 0.38, body, ha="center", va="center", color=palette["ink"], fontsize=9.1, transform=ax.transAxes)
    if i < len(cards) - 1:
        ax.annotate("", xy=(x + 0.123, 0.68), xytext=(x + 0.084, 0.68), xycoords=ax.transAxes, arrowprops=dict(arrowstyle="->", lw=1.8, color=palette["slate"]))
ax.set_title("Validation chain preserved by this public refresh", loc="left", fontsize=14.5, fontweight="bold")
fig.tight_layout()
fig.savefig(CONTROL_VIS_DIR / "03_validation_chain.png", dpi=180, bbox_inches="tight")
plt.show()
...
ax.axis("off")
ax.text(0.02, 0.82, "Exact archive produced by this Kaggle run", fontsize=14, fontweight="bold", color=palette["ink"], transform=ax.transAxes)
ax.text(0.02, 0.63, f"SHA256: {archive_sha[:24]}…{archive_sha[-12:]}", fontsize=10.5, color=palette["ink"], transform=ax.transAxes, family="monospace")
ax.text(0.02, 0.47, f"Members: {len(archive_members)} · deck rows: {len(deck_ids)} · smoke: {smoke}", fontsize=10.5, color=palette["ink"], transform=ax.transAxes)
ax.text(0.02, 0.29, "No official competition submission was created by this notebook run.", fontsize=10.5, color=palette["red"], transform=ax.transAxes, fontweight="bold")
ax.text(0.02, 0.12, "The correct next gate would be exact downloaded-output validation before any UI-only submission.", fontsize=10.0, color=palette["slate"], transform=ax.transAxes)
fig.tight_layout()
fig.savefig(CONTROL_VIS_DIR / "04_archive_receipt_card.png", dpi=180, bbox_inches="tight")
plt.show()

az_df = pd.DataFrame(
    [
        {"opponent": opponent.replace("_", " "), "win_rate": win_rate}
        for opponent, win_rate in AZUMARILL_LOCAL_PANEL["per_opponent_win_rates"].items()
    ]
).sort_values("win_rate")
az_df.to_csv(WORK / "ptcg_control_v11_azumarill_local_panel_20260730.csv", index=False)
fig, ax = plt.subplots(figsize=(12.8, 5.7))
colors = [palette["green"] if value >= 0.75 else palette["orange"] if value >= 0.60 else palette["slate"] for value in az_df["win_rate"]]
ax.barh(az_df["opponent"], az_df["win_rate"], color=colors)
ax.axvline(0.5, color=palette["red"], lw=1.6, linestyle="--", alpha=0.8)
ax.axvline(AZUMARILL_LOCAL_PANEL["raw_win_rate"], color=palette["blue"], lw=2.0, alpha=0.8)
ax.set_xlim(0.45, 1.0)
ax.set_xlabel("local SDK win rate")
ax.set_title("Radical successor candidate: Azumarill/Jellicent local proxy panel", loc="left", fontsize=14.5, fontweight="bold")
for y, row in enumerate(az_df.to_dict("records")):
    ax.text(row["win_rate"] + 0.01, y, f"{row['win_rate']:.1%}", va="center", fontsize=9)
ax.text(AZUMARILL_LOCAL_PANEL["raw_win_rate"] + 0.007, len(az_df) - 0.55, "aggregate 76.0%", color=palette["blue"], fontsize=9, fontweight="bold")
ax.grid(axis="x", alpha=0.22)
ax.spines[["top", "right"]].set_visible(False)
fig.tight_layout()
fig.savefig(CONTROL_VIS_DIR / "05_azumarill_local_proxy_panel.png", dpi=180, bbox_inches="tight")
plt.show()

fig, ax = plt.subplots(figsize=(12.6, 4.5))
ax.axis("off")
ax.text(0.02, 0.84, "Candidate status: promising, but fail-closed", fontsize=15, fontweight="bold", color=palette["ink"], transform=ax.transAxes)
ax.text(0.02, 0.66, f"{AZUMARILL_LOCAL_PANEL['wins']}W / {AZUMARILL_LOCAL_PANEL['losses']}L / {AZUMARILL_LOCAL_PANEL['draws']}D over {AZUMARILL_LOCAL_PANEL['games']} local SDK games · errors {AZUMARILL_LOCAL_PANEL['errors']}", fontsize=10.8, color=palette["ink"], transform=ax.transAxes)
ax.text(0.02, 0.50, f"Minimum opponent win rate: {AZUMARILL_LOCAL_PANEL['minimum_opponent_win_rate']:.1%} · deck rows: {AZUMARILL_LOCAL_PANEL['deck_rows']}", fontsize=10.8, color=palette["ink"], transform=ax.transAxes)
ax.text(0.02, 0.34, f"Archive SHA: {AZUMARILL_LOCAL_PANEL['archive_sha256'][:24]}…{AZUMARILL_LOCAL_PANEL['archive_sha256'][-12:]}", fontsize=9.8, color=palette["slate"], family="monospace", transform=ax.transAxes)
ax.text(0.02, 0.16, "Local proxy evidence is not a Kaggle score. Public run + exact downloaded-output validation are still required before any UI-only official submission.", fontsize=10.2, color=palette["red"], transform=ax.transAxes, fontweight="bold")
fig.tight_layout()
fig.savefig(CONTROL_VIS_DIR / "06_azumarill_fail_closed_card.png", dpi=180, bbox_inches="tight")
plt.show()

garchomp_rows = [
    {"comparison": "vs exact 844 parent", "schedule": "202607300101", "candidate_wins": 265, "control_wins": 237, "delta_pp": 7.2916666666666625, "min_anchor_delta_pp": -35.9375},
    {"comparison": "vs exact 844 parent", "schedule": "202607300102", "candidate_wins": 280, "control_wins": 216, "delta_pp": 16.666666666666664, "min_anchor_delta_pp": -20.3125},
    {"comparison": "vs exact V22 contextual", "schedule": "pooled", "candidate_wins": 552, "control_wins": 565, "delta_pp": -1.692708333333337, "min_anchor_delta_pp": -30.46875},
    {"comparison": "four-Roserade Weight vs Garchomp base", "schedule": "202607300130", "candidate_wins": 279, "control_wins": 255, "delta_pp": 6.25, "min_anchor_delta_pp": -1.5625},
    {"comparison": "four-Roserade Weight vs Garchomp base", "schedule": "202607300131", "candidate_wins": 270, "control_wins": 261, "delta_pp": 2.34375, "min_anchor_delta_pp": -6.25},
    {"comparison": "Stamp guard replication", "schedule": "pooled", "candidate_wins": 541, "control_wins": 532, "delta_pp": 1.171875, "min_anchor_delta_pp": -6.25},
]
garchomp_df = pd.DataFrame(garchomp_rows)
garchomp_df.to_csv(WORK / "ptcg_control_v11_garchomp_hold_panel_20260730.csv", index=False)
display(garchomp_df)

fig, ax = plt.subplots(figsize=(13.6, 5.2))
colors = [palette["green"] if d > 0 and m >= -2.0 else palette["orange"] if d > 0 else palette["red"] for d, m in zip(garchomp_df["delta_pp"], garchomp_df["min_anchor_delta_pp"])]
labels = garchomp_df["comparison"] + "\n" + garchomp_df["schedule"].astype(str)
ax.bar(labels, garchomp_df["delta_pp"], color=colors)
ax.axhline(0, color=palette["ink"], lw=1.2)
ax.set_title("Garchomp lane: promising local signal, but promotion gates stay fail-closed", loc="left", fontsize=14.5, fontweight="bold")
ax.set_ylabel("candidate minus control win-rate delta (pp)")
ax.tick_params(axis="x", rotation=18)
ax.grid(axis="y", alpha=0.22)
for x, row in enumerate(garchomp_df.to_dict("records")):
    ax.text(x, row["delta_pp"] + (0.55 if row["delta_pp"] >= 0 else -1.2), f"{row['delta_pp']:+.1f} pp", ha="center", va="bottom" if row["delta_pp"] >= 0 else "top", fontsize=9)
ax.spines[["top", "right"]].set_visible(False)
fig.tight_layout()
fig.savefig(CONTROL_VIS_DIR / "07_garchomp_local_hold_panel.png", dpi=180, bbox_inches="tight")
plt.show()

fig, ax = plt.subplots(figsize=(12.8, 4.8))
anchors = pd.Series(GARCHOMP_LOCAL_PANEL["vs_exact_v22_contextual"]["per_anchor_delta_pp"]).sort_values()
anchor_colors = [palette["red"] if value < -5 else palette["green"] if value > 5 else palette["slate"] for value in anchors]
ax.barh([name.replace("_", " ") for name in anchors.index], anchors.values, color=anchor_colors)
ax.axvline(0, color=palette["ink"], lw=1.1)
ax.set_title("Why the Garchomp lane is not promoted yet: exact V22 anchor deltas", loc="left", fontsize=14.5, fontweight="bold")
ax.set_xlabel("candidate minus V22 contextual local win-rate delta (pp)")
for y, value in enumerate(anchors.values):
    ax.text(value + (0.8 if value >= 0 else -0.8), y, f"{value:+.1f}", va="center", ha="left" if value >= 0 else "right", fontsize=9)
ax.grid(axis="x", alpha=0.22)
ax.spines[["top", "right"]].set_visible(False)
fig.tight_layout()
fig.savefig(CONTROL_VIS_DIR / "08_garchomp_anchor_regression_panel.png", dpi=180, bbox_inches="tight")
plt.show()

visuals = sorted(str(path.relative_to(WORK)) for path in CONTROL_VIS_DIR.glob("*.png"))
archive_sha = hashlib.sha256(archive.read_bytes()).hexdigest()
receipt = {
    "status": "PUBLIC_CONTROL_V11_REFRESH_PASS",
    "notebook_slug": "prvsiyan/ptcg-ai-battle-control-v11-meta-portfolio",
    "refresh_utc": REFRESH_UTC,
    "scope": "existing public CPU scoring-archive builder; visual/evidence addendum; policy and deck cells unchanged",
    "exact_official_rows": CURRENT_OFFICIAL_ROWS,
    "azumarill_local_candidate_panel": AZUMARILL_LOCAL_PANEL,
    "garchomp_local_hold_panel": GARCHOMP_LOCAL_PANEL,
    ...
    "identity_guard": {
        "id": "prvsiyan/ptcg-ai-battle-control-v11-meta-portfolio",
        "title": "PTCG AI Battle: Control v11 Meta Portfolio",
        "code_file": "ptcg-ai-battle-control-v11-meta-portfolio.ipynb",
        "delete_action_allowed": False,
    },
}
(WORK / "ptcg_control_v11_refresh_receipt_20260730.json").write_text(json.dumps(receipt, indent=2, sort_keys=True) + "\n")
assert len(deck_ids) == 60
assert len(visuals) == 8, visuals
assert archive.exists()
print(json.dumps({"status": "PASS", "visual_count": len(visuals), "deck_rows": len(deck_ids), "submission_tar_gz_sha256": archive_sha, "official_competition_submission_created": False}, indent=2))
