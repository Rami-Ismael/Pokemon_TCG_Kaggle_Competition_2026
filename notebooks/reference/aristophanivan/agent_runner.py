"""
Adapted, RUNNABLE port of Ivan Ternovskiy's Improved Probabilistic Agent.

Source: https://www.kaggle.com/code/aristophanivan/improved-probabilistic-agent

This file keeps the original agent logic VERBATIM (deck list, heuristics,
UCB1 search) and adapts ONLY the calls that the current local `cg` SDK
changed since the Kaggle notebook was written:

  ADAPTED 1: `search_begin` now requires six prediction arguments
             (your deck/prize, opponent deck/prize/hand/active) instead of the
             single `your_deck=` the original passed.
  ADAPTED 2: `SearchState` exposes the id as `.searchId` (the original read
             `.id`).

Everything labelled ADAPTED is the minimum change needed to execute here; the
policy/scoring/search math is unchanged from the source.
"""
...
_SEARCH_OK = True  # local SDK always exposes search_*
USE_SEARCH = True
...
with open(DECK_PATH, "r", encoding="utf-8") as f:
    my_deck = [int(l) for l in f.read().splitlines() if l.strip()]
...
def rollout_turn(sid, cur_obs, your_index):
    ...
    while steps < 20:
        ...
        if ar is None or ar.observation is None:
            break
        cur_obs, sid = ar.observation, getattr(ar, "searchId", None)  # local SDK: SearchState
        steps += 1
    return cur_obs


# ADAPTED: helper that builds the 6 prediction lists the local SDK needs.
def _predict(obs: Observation) -> dict:
    """Build the six card-ID predictions search_begin() now requires.

    We cannot see hidden opponent cards, so we predict with the deck list we
    know (our own known deck) padded to the required lengths. This is a
    *prediction*, exactly what the engine expects; it does not need to be
    correct, only the right length and containing at least one Basic Pokémon
    on the opponent's side at setup.
    """
    st = obs.current
    ...
    your_deck = pad(my_deck, me.deckCount)
    your_prize = pad(my_deck, len(me.prize))
    opponent_deck = pad(my_deck, op.deckCount)
    opponent_prize = pad(my_deck, len(op.prize))
    opponent_hand = pad(my_deck, op.handCount)

    # Only needed if opponent's active is face-down (None); otherwise ignored.
    opp_active = []
    if op.active and op.active[0] is None:
        opp_active = [673]  # a known Basic (Makuhita) as a stand-in prediction
    return dict(
        your_deck=your_deck,
...
def simulate_action(obs, action):
    preds = _predict(obs)
    sbi = search_begin(obs, **preds)  # ADAPTED: 6 args instead of your_deck= only
    if sbi is None or sbi.observation is None:
        return -float("inf")
    ar = search_step(getattr(sbi, "searchId", None), [action])  # local SDK
    if ar is None or ar.observation is None:
        return -float("inf")
    cur = rollout_turn(getattr(ar, "searchId", None), ar.observation, obs.current.yourIndex)
...
__all__ = [
    "DECK", "my_deck", "card_table", "AdvancedPolicy", "attack_helpers",
    "evaluate_state", "simulate_action", "SEARCH_ALGO", "agent",
    "AttackPlan", "plan", "pre_turn", "ability_used",
]
