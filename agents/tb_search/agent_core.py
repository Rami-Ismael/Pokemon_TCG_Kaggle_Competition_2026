"""TomBombadyl SearchScorer scorer brain on real_mega_lucario_ex (mu 660.5 -- his best home-grown brain).

Vendored from github.com/TomBombadyl/kaggle_pokemon. His scorer brains are packaged by
`scripts/package_submission.py --scorer search`, which bundles the whole `agent/`
...
and builds the same scorer. `search` is a deck-agnostic hand-rules brain with a shallow cg SearchBegin (~200ms) on promote/switch/setup, falling back to HeuristicScorer.
