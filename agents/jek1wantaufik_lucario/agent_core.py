"""jek1wantaufik — rule-based Mega Lucario ex agent (benchmark mirror).

PROVENANCE
----------
Recovered from the Kaggle *model* `jek1wantaufik/buddy`
(other/pokemon/1/main.py + deck.pkl), which is the actual playable agent
referenced — but never printed — by the notebook
"Pokémon TCG AI Strategy Analysis"
(https://www.kaggle.com/code/jek1wantaufik/pok-mon-tcg-ai-strategy-analysis).
That notebook only *statically analyses* this file (AST counts of functions,
branches, heuristic categories) and generates a prose "strategy report"; it
contains no runnable decision logic of its own. See
notebooks/reference/jek1wantaufik-strategy-analysis/NOTES.md for the full
trail.

WHAT THIS AGENT IS
------------------
A pure hand-written heuristic (NO search / NO learning). Each decision it
scores every legal option and returns them best-first, truncated to
`select.maxCount`. On a MAIN turn it first commits to a single `plan`
(attacker, target, attack, prize trade, whether it still needs an energy),
then every downstream scorer — attach, switch, retreat, gust, evolve — lines
up behind that plan. Same Mega Lucario ex archetype as our `rule_baseline`
and `dedquoc_rule_engine`, but an independent implementation with its own
scoring constants (prize×1000, energies×150, lethal=50000, no
water/Crustle special-casing), so it belongs in the pool as a distinct
opponent, not a duplicate.

FIDELITY
--------
The `agent()` decision logic below is byte-for-byte the recovered `main.py`.
The ONLY change is the deck-loading preamble: the original unconditionally
`pickle.load`s `deck.pkl` (and crashes on import if it is absent). This repo
wires decks via a plain-text `deck.csv` (see scripts/benchmark_agents.py
`load_agent`), so the preamble here prefers `deck.csv`, then the original
`deck.pkl`, then a hardcoded fallback — and never crashes at import. No
scoring constant, threshold, or control-flow branch was touched.
"""
...
# 60-card Mega Lucario ex list, decoded from the original deck.pkl. Used only
# if neither deck.csv nor deck.pkl can be read, so the module always imports.
_DECK_FALLBACK = [
...
def _load_deck() -> list[int]:
    """Prefer this repo's deck.csv, then the original deck.pkl, then fallback.

    Checks the module directory and the Kaggle sandbox mount for each format.
    Any deck that does not parse to exactly 60 ids is rejected in favour of the
    hardcoded list, so a bad file can never silently ship a short deck.
    """
    here = os.path.dirname(os.path.abspath(__file__))
    for csv_path in (os.path.join(here, "deck.csv"), "deck.csv",
        ...
        try:
            ...
            pass
    for pkl_path in (os.path.join(here, "deck.pkl"), "deck.pkl",
                     "/kaggle_simulations/agent/deck.pkl"):
        try:
