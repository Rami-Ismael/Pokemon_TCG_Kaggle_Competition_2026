"""TomBombadyl RuleCoreScorer scorer brain on real_mega_lucario_ex (mu 535.6).

Vendored from github.com/TomBombadyl/kaggle_pokemon. His scorer brains are packaged by
`scripts/package_submission.py --scorer rulecore`, which bundles the whole `agent/`
...
and builds the same scorer. `rulecore` is a deck-agnostic hand-rules brain (deck-agnostic attack plan from engine tables + deck_tech, falling back to HeuristicScorer).
...
from agent.rule_core import RuleCoreScorer  # noqa: E402
_AGENT = build_agent(seed=0, deck_path=_DECK, scorer=RuleCoreScorer(deck_path=_DECK))
