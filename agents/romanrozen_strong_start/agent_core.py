"""romanrozen strong-start baseline (Kaggle LB ~950), wired for benchmark_agents.py.
Source: notebooks/reference/romanrozen-strong-start-agent (verbatim main.py cell;
deck.csv write + deck-read block + submission-packaging cells stripped -- my_deck is
injected from this dir's deck.csv by benchmark_agents.load_agent, matching the other
wired reference agents). Byte-identical to aristophanivan/improved-probabilistic-agent
(same lineage), so only this copy is wired. Probabilistic Expectimax policy with a
UCB1/MCTS re-ranker over real cg engine search rollouts (search_begin/search_step).
Mega Lucario ex (Fighting) deck. Returns list[int] option indices.
"""
