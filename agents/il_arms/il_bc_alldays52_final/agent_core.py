...
models/bc_alldays52_jun16_aug07_seed42 (FINAL checkpoint, step 736,715 --
not the best-eval snapshot at 620k, which lives in
models/bc_alldays52_jun16_aug07_seed42_best).
...
_CKPT = _REPO / "models" / "bc_alldays52_jun16_aug07_seed42"
_CORE = _REPO / "agents" / "il_agent" / "agent_core.py"

_prev = os.environ.get("IL_MODEL_DIR")
os.environ["IL_MODEL_DIR"] = str(_CKPT)
try:
    _spec = importlib.util.spec_from_file_location("il_core_il_bc_alldays52_final", _CORE)
...
# Re-export the fallback-diagnostic interface so the benchmark harness
# (which checks hasattr(module, "diag_snapshot") on THIS wrapper module)
# can report per-arm fallback rates -- without this, a silent fallback in
# the core would be invisible for deck-tagged arms.
