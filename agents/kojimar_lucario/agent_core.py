"""Mega Lucario ex rule-based agent — reimplementation of kojimar's
"Simple Baseline + Matchup Tests" notebook (baseline exp00014_20260617_202758).

Source: https://www.kaggle.com/code/kojimar/simple-baseline-matchup-tests
Mirror: notebooks/reference/kojimar-simple-baseline/

Wired as a bare module following the repo convention (cf. makthanithin_improved_prob):
the notebook's `def agent()` body is kept verbatim; the deck-file round-trip
(`deck.csv` -> `my_deck`) is replaced by the notebook's own literal `DECK` constant
assigned directly to `my_deck`. The build/archive/matchup-table/smoke-test cells are
dropped (they are packaging + local validation, not agent logic).

This is a *distinct* Mega Lucario ex list from agents/mega_lucario/deck.csv — it differs
by 5 slots: +1 Basic Fighting Energy, +1 Riolu, +1 Boss's Orders, -2 Poké Pad,
-1 Gravity Mountain. The policy already does lightweight opponent-archetype inference
(_opponent_is_water_deck / _opponent_is_crustle_wall), which the deck-identification
experiment (scripts/identify_opponent_deck.py) generalizes into a full sniffer.
"""
...
# Deck: the notebook's literal DECK constant (60 cards, Mega Lucario ex).
DECK = [
