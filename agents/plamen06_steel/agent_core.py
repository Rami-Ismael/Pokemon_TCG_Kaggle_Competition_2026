# Archaludon metal-tempo: heuristic + Kaggle-safe optional beam search
...
try:
    ...
    ROOT = None
# Add whatever directory actually contains the cg/ SDK to sys.path.
# Works at Kaggle runtime (/kaggle_simulations/agent) AND in the notebook
# (cwd / /kaggle/working, once the build cell has copied cg/ there).
_cg_candidates = []
if ROOT:
    try:
        _cg_candidates.append(os.path.dirname(os.path.abspath(ROOT)))
    except Exception:
        pass
_cg_candidates += ["/kaggle_simulations/agent", os.getcwd(), "/kaggle/working", "."]
for p in _cg_candidates:
    if p and p not in sys.path and os.path.isdir(os.path.join(p, "cg")):
        sys.path.insert(0, p)
...
# Ported from the original notebook's read_deck_csv(), which read a relative
# "deck.csv" path -- fragile under a harness whose cwd isn't the bundle dir
# (a repo-root deck.csv exists here for a different agent's frozen deck, and
# a relative read would have silently loaded THAT deck instead of this
# agent's Archaludon/Cinderace one). Matches every other bare-module agent
# in this repo (see agents/kiyotah_dragapult/agent_core.py): `my_deck` is
# injected externally by main.py or by scripts/benchmark_agents.py's loader
# from the sibling deck.csv, never read from a relative path at call time.
def read_deck_csv():
    return my_deck
...
# =====================================================================
# Optional beam-search lookahead (MultiPly-style, Kaggle-safe).
# Uses ONLY the high-level cg.api search functions (search_begin(sbi),
# search_step). It does NOT touch cg.sim.lib / ctypes, so a mismatched
# engine can raise a normal exception (caught) but can never segfault.
# If the search API is unavailable or errors, we fall back to the proven
# heuristic (choose_options). Tunables via env for experimentation.
# =====================================================================
import os as _os
_SEARCH_OK = False
try:
    ...
    _SEARCH_OK = False
ENABLE_SEARCH = True   # set False for pure-heuristic (safest) submission
USE_SEARCH = ENABLE_SEARCH and _os.environ.get("USE_SEARCH", "1") == "1"
CAND   = int(_os.environ.get("BEAM_CAND", "6"))
MAXD   = int(_os.environ.get("BEAM_MAXD", "40"))
MARGIN = float(_os.environ.get("BEAM_MARGIN", "4000"))
import time as _time
BUDGET = float(_os.environ.get("SP_BUDGET", "2.0"))
_MAIN = SelectContext.MAIN

def _en(pk):
    try: return energy_count(pk)
    except Exception: return len(getattr(pk, "energyCards", None) or [])

def evaluate_state(obs, me):
    st = obs.current
    if st is None: return 0.0
    mp = st.players[me]; op = st.players[1 - me]
    if len(mp.prize) == 0: return 9e6
    if len(op.prize) == 0: return -9e6
    val = (len(op.prize) - len(mp.prize)) * 10000.0
    for p in ([mp.active[0]] if mp.active else []) + list(mp.bench):
        if p is None: continue
        val += _en(p) * 200.0
        if p.id == ARCHALUDON_EX: val += 500.0
        elif p.id == DURALUDON: val += 200.0
    if mp.active and mp.active[0] is not None:
        a = mp.active[0]; val += a.hp * 2.0
        if a.id == ARCHALUDON_EX and _en(a) >= 3: val += 800.0
    if op.active and op.active[0] is not None:
        val -= op.active[0].hp * 2.5
    val += getattr(mp, "handCount", 0) * 10.0
    if getattr(mp, "deckCount", 60) < 5: val -= 5000.0
    return val

def _ranked(obs):
    scored = []
    for i, opt in enumerate(obs.select.option):
        try: s, _ = score_option(obs, opt)
...
def _rollout(sid, obs, me):
    ...
    while (cur_obs.select is not None and cur_obs.current.result is not None
           and cur_obs.current.result == -1 and cur_obs.current.yourIndex == me
           and cur_obs.current.turn == start_turn and depth < MAXD):
        try: act = choose_options(cur_obs)
        except Exception: break
        try: ar = search_step(sid, act)
        except Exception: break
        if getattr(ar, "error", 0) != 0 or ar.state is None: break
        cur_obs = ar.state.observation; sid = ar.state.searchId; depth += 1
    return cur_obs

def _line_value(cur, me, endobs):
    if cur.result == me: return (2, 6, 9e6)
    if cur.result == (1 - me): return (-1, 0, -9e6)
    prizes = 6 - len(cur.players[me].prize)
    return (0, prizes, evaluate_state(endobs, me))

def _search_choose(obs_dict, obs):
    sbi = obs_dict.get("search_begin_input") or getattr(obs, "search_begin_input", None)
    if not sbi: return None
    heur = choose_options(obs); hidx = heur[0] if heur else 0
    res = search_begin(sbi)
    if getattr(res, "error", 0) != 0 or getattr(res, "state", None) is None: return None
    ...
    if (vals[bp][0], vals[bp][1]) > (h[0], h[1]): return [bp]
    be = max(vals, key=lambda i: vals[i][2])
    if vals[be][2] > h[2] + MARGIN and (vals[be][0], vals[be][1]) >= (h[0], h[1]): return [be]
    return heur
...
def agent(obs_dict):
    ...
    if obs.select is None:
        global _opp_last_attack_id, _cur_turn_logs
        _opp_last_attack_id = None; _cur_turn_logs.clear()
        return read_deck_csv()
    try: _update_opp_attack_tracking(obs)
    except Exception: pass
    if not obs.select.option: return []
    sel = obs.select
    if (_SEARCH_OK and USE_SEARCH and sel.context == _MAIN
            and sel.minCount == 1 and sel.maxCount == 1 and len(sel.option) >= 2):
        try:
