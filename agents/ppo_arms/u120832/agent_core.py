"""Stage-3 PPO candidate wrapper: il_agent inference core, checkpoint u120832.
...
_CKPT = _REPO / "models" / "ppo_puffer" / "u120832"
_CORE = _REPO / "agents" / "il_agent" / "agent_core.py"

_prev = os.environ.get("IL_MODEL_DIR")
os.environ["IL_MODEL_DIR"] = str(_CKPT)
try:
    _spec = importlib.util.spec_from_file_location("il_core_ppo_u120832", _CORE)
