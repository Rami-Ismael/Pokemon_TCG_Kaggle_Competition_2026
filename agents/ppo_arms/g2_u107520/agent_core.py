"""Stage-3 PPO candidate wrapper: il_agent inference core, checkpoint g2 u107520_final (gamma 0.997 + KL anchor).

Same pattern as agents/s2_arms/: loads agents/il_agent/agent_core.py as a
private module with IL_MODEL_DIR pointed at models/ppo_puffer/u120832, env var
...
_CKPT = _REPO / "models" / "ppo_puffer_g2" / "u107520_final"
_CORE = _REPO / "agents" / "il_agent" / "agent_core.py"

_prev = os.environ.get("IL_MODEL_DIR")
os.environ["IL_MODEL_DIR"] = str(_CKPT)
try:
    _spec = importlib.util.spec_from_file_location("il_core_ppo_g2_u107520", _CORE)
